...
public class RootDotenvEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        addDatabaseVariableAliases(environment, dotenvValues);
        addSpringDatasourceAliases(environment, dotenvValues);
        if (dotenvValues.isEmpty()) {
            return;
        }

        addDotenvSystemProperties(environment, dotenvValues);
    ...
    private static void addDatabaseVariableAliases(ConfigurableEnvironment environment, Map<String, Object> values) {
        putBidirectionalAliasIfAbsent(environment, values, "DB_URL", "DABOYEO_DB_URL");
        putBidirectionalAliasIfAbsent(environment, values, "DB_USERNAME", "DABOYEO_DB_USERNAME");
        putBidirectionalAliasIfAbsent(environment, values, "DB_PASSWORD", "DABOYEO_DB_PASSWORD");
    }

    private static void putBidirectionalAliasIfAbsent(
        ConfigurableEnvironment environment,
        Map<String, Object> values,
        String firstKey,
        String secondKey
    ) {
        putAliasIfAbsent(environment, values, firstKey, secondKey);
        putAliasIfAbsent(environment, values, secondKey, firstKey);
    }

    private static void addSpringDatasourceAliases(ConfigurableEnvironment environment, Map<String, Object> values) {
        putAliasIfAbsent(environment, values, "DB_URL", "spring.datasource.url");
        putAliasIfAbsent(environment, values, "DB_USERNAME", "spring.datasource.username");
        putAliasIfAbsent(environment, values, "DB_PASSWORD", "spring.datasource.password");
    }
    ...
    private static void addDotenvSystemProperties(ConfigurableEnvironment environment, Map<String, Object> values) {
        values.forEach((key, value) -> {
            if (value == null || hasHighPriorityProperty(environment, key)) {
                return;
            }
