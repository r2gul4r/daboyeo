...
public class RecommendationProfileRepository {
    ...
    public Optional<RunFeedbackContext> findRunFeedbackContext(String runId) {
        return jdbcTemplate.query(
            """
            SELECT anonymous_id, response_json
            ...
            rs -> {
                ...
                return Optional.of(new RunFeedbackContext(
                    rs.getString("anonymous_id"),
                    extractRecommendedShowtimeIds(rs.getString("response_json"))
                ));
            },
            runId
        );
    ...
    private Set<Long> extractRecommendedShowtimeIds(String responseJson) {
        ...
        try {
            ...
            if (recommendations.isArray()) {
                recommendations.forEach(item -> {
                    JsonNode showtimeId = item.path("showtimeId");
                    if (showtimeId.canConvertToLong()) {
                        ids.add(showtimeId.longValue());
                    }
                });
            }
    ...
    public record RunFeedbackContext(String anonymousId, Set<Long> recommendedShowtimeIds) {
        public RunFeedbackContext {
            anonymousId = anonymousId == null ? "" : anonymousId.trim();
            recommendedShowtimeIds = recommendedShowtimeIds == null ? Set.of() : Set.copyOf(recommendedShowtimeIds);
        }
    }
}
