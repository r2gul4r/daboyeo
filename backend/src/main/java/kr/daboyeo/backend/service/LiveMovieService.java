...
public class LiveMovieService {
        ...
        try {
            ...
            if (results.isEmpty() && liveFallbackEnabled) {
                return nearbyResponse(
                    criteria,
                    toItems(demoDataService.sampleSchedules(criteria), criteria),
                    false,
                    DEMO_WARNING
                );
            }
            return nearbyResponse(criteria, results, true, null);
            ...
            return nearbyResponse(criteria, List.of(), false, "database lookup failed.");
        }
    }
    ...
    public MovieSchedulesResponse findMovieSchedules(String movieKey, LiveMovieSearchCriteria criteria) {
        try {
            ...
            if (items.isEmpty() && liveFallbackEnabled) {
                List<LiveMovieScheduleItem> demoItems = toItems(
                    demoDataService.sampleSchedules(criteria).stream()
                        .filter(schedule -> schedule.movieKey().equalsIgnoreCase(movieKey))
                        .toList(),
                    criteria
                );
                return schedulesResponse(movieKey, criteria, demoItems, false, DEMO_WARNING);
            }
            return schedulesResponse(movieKey, criteria, items, true, null);
        } catch (DataAccessException exception) {
            if (liveFallbackEnabled) {
                List<LiveMovieScheduleItem> demoItems = toItems(
                    demoDataService.sampleSchedules(criteria).stream()
            ...
            return schedulesResponse(movieKey, criteria, List.of(), false, "database lookup failed.");
        }
    }
