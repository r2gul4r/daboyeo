...
- task: `High-performance Lotte Cinema crawler refactoring and high-availability architecture`
- phase: `verified`
- scope: `Refactor monolithic Lotte crawler into modular interface-based components with parallel execution, connection pooling, retry policies, metrics tracking, and batch synchronization.`
- verification_target: `LotteCinemaEventCrawler uses BaseMovieEventCrawler interface, Parallel category crawling via CompletableFuture, RestTemplate with HttpClient5 connection pooling, LotteCinemaApiClient with retry/charset fallback, CrawlingMetrics for observability, MovieEventBatchService for sync, and MovieEventSyncScheduler for orchestration. All tests pass.`
- classification: `score_total=9; single-session; orchestration_value=medium; agent_budget=0; evaluation_need=full`
- reason: `Large architectural change involving multiple components, performance optimizations, and operational stability. Hard triggers: crawler architecture, concurrency, DB sync strategy, and scheduler.`
- write_sets: `STATE.md; backend/src/main/java/kr/daboyeo/backend/crawler/**; backend/src/main/java/kr/daboyeo/backend/service/MovieEventBatchService.java; backend/src/main/java/kr/daboyeo/backend/sync/MovieEventSyncScheduler.java; backend/src/main/java/kr/daboyeo/backend/config/LotteCinemaConfig.java; backend/build.gradle`
- contract_freeze: `Use BaseMovieEventCrawler as the common interface. Parallelize category crawling. Implement connection pooling in RestTemplate. Use AtomicBoolean for scheduler locking. Maintain existing data mapping logic but split into Parser.`
- verification_evidence: `LotteCinemaCrawlerTest passed with 5 tests (including parallel execution). Sync logic verified via MovieEventBatchService code review. Connection pooling verified via RestTemplate config. Metrics and structured logging integrated.`

## Retrospective

- task: `Enforce MovieEvent uniqueness by eventId and cinema`
- score_total: `6`
- evaluation_fit: `light fit; the change is a bounded JPA/migration/save-path idempotency fix`
- orchestration_fit: `single-session fit; entity, crawler mapping, repository check, and batch save logic are tightly coupled`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `legacy migration backfill changed to legacy:id so existing duplicate rows cannot block unique key creation`
- reviewer_findings: `DB unique constraint is the final duplicate guarantee; service keeps batch insert through saveAll while skipping null keys, repeated batch keys, and existing DB keys with debug logs`
- verification_outcome: `compileJava passed; git diff --check passed with CRLF warnings only`
- next_rule_change: `when adding uniqueness to a table that may already contain duplicates, make migration backfills row-unique before enforcing the new provider identity contract`
- score_total: `6`
- evaluation_fit: `light fit; the change is a bounded JPA/migration/save-path idempotency fix`
- orchestration_fit: `single-session fit; entity, crawler mapping, repository check, and batch save logic are tightly coupled`
- predicted_topology: `single-session`
...
- task: `Add required cinema field to MovieEvent persistence flow`
- score_total: `6`
- evaluation_fit: `light fit; the change is a JPA mapping and save-path guard for a known NOT NULL column`
- orchestration_fit: `single-session fit; entity, crawler constructor call, and service guard are tightly coupled`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `repo migration V005 does not include cinema, but the live DB error proves the active table has a NOT NULL cinema column, so no migration was added to avoid schema collision`
- reviewer_findings: `MovieEvent now maps cinema nullable=false; legacy constructor defaults cinema to source; Lotte crawler passes LOTTE explicitly; MovieEventService skips null or blank cinema before repository.save`
- verification_outcome: `compileJava passed; git diff --check passed with CRLF warnings only`
- next_rule_change: `when live DB errors mention an unmapped NOT NULL column, inspect both migration history and runtime schema before deciding whether to add a migration or only update JPA mapping`

- task: `Switch Lotte event crawler to discovered LCWS JSON XHR endpoint`
- score_total: `6`
- evaluation_fit: `light fit; the runtime failure was a narrow wrong-endpoint/content-type handling issue`
- orchestration_fit: `single-session fit; one crawler owns request headers, LCWS payload construction, raw body logging, JSON gating, and DTO parsing`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `the page URL was confirmed to be a React shell; EventDetailList.bundle.js uses LCWS event POST with MethodName=GetEventLists, so the crawler moved from page URL GET to API POST`
- reviewer_findings: `HTML page URL is no longer used as data source; Referer remains the category page; LCWS response is parsed by JSON-looking body because Lotte returns text/html content-type for JSON; DTO and MovieEvent mapping remain intact`
- verification_outcome: `live endpoint smoke returned Items JSON; compileJava passed`
- next_rule_change: `when a provider page is JS-rendered, inspect the page bundle/Common.js URL map before writing crawler URLs`

- task: `Refactor Lotte event crawler from HTML parsing to JSON API mapping`
- score_total: `6`
- evaluation_fit: `light fit; the change is a localized crawler parser swap with compile verification`
- orchestration_fit: `single-session fit; one file owns the request, DTO mapping, and MovieEvent conversion`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `DTOs were kept inside the crawler and marked ignoreUnknown so real API responses can include extra fields without breaking RestTemplate mapping`
- reviewer_findings: `DOM parsing is gone, EventName is cleaned through Jsoup.parse().text(), ImageAlt can fill blank EventName, dates parse from ProgressStartDate/ProgressEndDate, and category URL/log/error behavior is preserved`
- verification_outcome: `compileJava passed; static search found no old HTML parsing path; git diff --check passed with CRLF warnings only`
- next_rule_change: `when switching provider HTML crawlers to JSON, add ignoreUnknown DTOs by default because provider API rows usually include more fields than the local contract needs`

- task: `Make Python collector bridge JSON-safe and diagnosable`
- score_total: `7`
- evaluation_fit: `full fit; subprocess JSON safety needed focused tests for valid output, no output, Python exception, and invalid JSON`
- orchestration_fit: `single-session fit; all fixes were centralized in PythonCollectorBridge and a focused test file`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `the existing bridge used output files rather than stdout as source of truth, so the fix became a JSON-safe Python wrapper plus Java-side parse/log guards instead of editing every collector script`
- reviewer_findings: `successful collector payload shape is preserved; failure/no-output paths now return success=false/data=[]/error JSON; stdout from user scripts is redirected to stderr so wrapper stdout remains JSON; logs use bounded previews and do not dump env/secrets`
- verification_outcome: `PythonCollectorBridgeTests passed; git diff --check passed with CRLF warnings only`
- next_rule_change: `for subprocess bridges, wrap foreign-language snippets once at the boundary and test blank/invalid/exception output before debugging individual collectors`

- task: `Load root .env into Spring Boot datasource configuration`
- score_total: `6`
- evaluation_fit: `light fit; the change is startup configuration plus focused post-processor tests`
- orchestration_fit: `single-session fit; all edits were one Spring config surface with no delegation value`
- predicted_topology: `single-session`
- actual_topology: `single-session`
- spawn_count: `0`
- rework_or_reclassification: `focused tests first failed because .env System property registration also preserved TIDB_* between tests; test cleanup was expanded to cover all keys the loader can set`
- reviewer_findings: `DB_URL/DB_USERNAME/DB_PASSWORD now drive datasource config, existing DABOYEO_DB_* and TIDB_* compatibility remains, and real .env secrets were not read or printed`
- verification_outcome: `focused Gradle dotenv tests passed; git diff --check passed with CRLF warnings only`
- next_rule_change: `for future startup env loaders, clear every key the loader writes during tests, not only the direct target aliases`
